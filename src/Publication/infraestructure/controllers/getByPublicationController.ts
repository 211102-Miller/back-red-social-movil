...
export class GetByPublicationController{
    constructor(readonly getByPublicationUseCase: GetByPublicationUseCase){}
    ...
    async getByPublic(req:Request,res:Response){
        try {
            let { uuid } = req.params; 
            console.log(uuid)
            let getBook = await this.getByPublicationUseCase.getPublic(uuid);
